...
public class ImageUtils {
	 ...
	 * @throws IOException
	 */
	public static boolean scaleImageByWidth(URL url, String dest, int maxSize) throws IOException {
		BufferedImage src = ImageIO.read(url); // 读入文件
		int w = src.getWidth();
		int h = src.getHeight();
		...
		Thumbnails.of(src).size(tow, toh).toFile(dest);
		return true;
	}
	...
	public static void scale(URL url, String dest, int width, int height) throws IOException {
		validate(dest);
		log.debug("scaled with/height : " + width + "/" + height);
		Thumbnails.of(url).size(width, height).toFile(dest);
	}
